...
module Backup
  module Compressor
    class Pigz < Base
      ...
      ##
      # Use the `--rsyncable` option with `pigz`.
      #
      # This option directs `pigz` to compress data using an algorithm that
      ...
      #
      # The `--rsyncable` option is only available on patched versions of `pigz`.
      ...
      def self.has_rsyncable?
        ...
        cmd = "#{ utility(:pigz) } --rsyncable --version >/dev/null 2>&1; echo $?"
        @has_rsyncable = %x[#{ cmd }].chomp == '0'
      end

      ##
      # Creates a new instance of Backup::Compressor::Pigz
      ...
      def options
        ...
        if self.class.has_rsyncable?
          ...
          Logger.warn Error.new(<<-EOS)
            ...
            Your system's 'pigz' does not support the `--rsyncable` option.
          EOS
        end if @rsyncable
        opts
      end
